...
async def _audit_sync_health(
    ...
    failed = (
        await session.execute(
            select(SyncRun)
            .where(SyncRun.status == SyncRunStatus.FAILED, SyncRun.started_at >= cutoff)
    ...
    for run in failed:
        findings.append(Finding(
            ...
            remediation="Check worker logs for adapter errors; verify API credentials and retry.",
            context={
                "run_id": run.id,
    ...
    for merchant in all_merchants:
        if merchant.id not in success_merchants:
            findings.append(Finding(
                ...
                remediation="Trigger sync manually or verify scheduled worker is running.",
                context={"merchant_slug": merchant.slug, "merchant_id": merchant.id},
            ))
    ...
    for slug, last_run in stale:
        ...
        last_run_aware = last_run.replace(tzinfo=timezone.utc) if last_run.tzinfo is None else last_run
        if last_run_aware < stale_cutoff:
            findings.append(Finding(
                severity=Severity.WARNING,
                category="sync.stale",
                message=f"Merchant '{slug}' last synced >7 days ago",
                remediation="Verify sync-worker scheduling; check adapter connectivity.",
...
async def _audit_data_quality(session: AsyncSession) -> list[Finding]:
    ...
    missing_cat = (
        await session.execute(
            select(func.count()).select_from(Product).where(
                Product.category.is_(None), Product.is_active.is_(True)
            )
        )
    ).scalar_one()
    ...
    if missing_cat > 0:
        findings.append(Finding(
            ...
            remediation="Review catalog adapter output; add category mapping in adapter config.",
    ...
    missing_desc = (
        await session.execute(
            select(func.count()).select_from(Product).where(
                Product.description.is_(None), Product.is_active.is_(True)
            )
        )
    ).scalar_one()
    ...
    if missing_desc > 0:
        findings.append(Finding(
            ...
            remediation="Enrich product descriptions via adapter or manual admin input.",
    ...
    null_price = (
        await session.execute(
            select(func.count()).select_from(Offer).where(
                Offer.price.is_(None), Offer.is_active.is_(True)
            )
        )
    ).scalar_one()
    ...
    if null_price > 0:
        findings.append(Finding(
            ...
            remediation="Verify adapter is returning prices; check partner API price field.",
...
async def _audit_content(session: AsyncSession) -> list[Finding]:
    ...
    if missing_excerpt > 0:
        findings.append(Finding(
            ...
            remediation="Review editorial workflow; add excerpts before publication.",
    ...
    if missing_meta > 0:
        findings.append(Finding(
            ...
            remediation="Generate meta descriptions via admin content API or manual review.",
    ...
    articles_without_links = (
        await session.execute(
            select(func.count()).select_from(Article).where(
                Article.status == ArticleStatus.PUBLISHED,
                ~Article.id.in_(
                    select(ArticleProduct.article_id).distinct()
                ),
            )
        )
    ).scalar_one()
    ...
    if articles_without_links > 0:
        findings.append(Finding(
            ...
            message=f"{articles_without_links} published articles with no product links",
            remediation="Run link suggestion generation for existing articles.",
